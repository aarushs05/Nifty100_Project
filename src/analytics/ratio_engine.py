...
from src.analytics.cagr import cagr_to_record, compute_all_cagr
from src.analytics.cashflow_kpis import compute_cashflow_metrics
...
class RatioEngine:
    ...
    def __init__(self):
        ...
        self.pnl = self.db.table("profitandloss")

        self.bs = self.db.table("balancesheet")

        self.cf = self.db.table("cashflow")

        self.company = self.db.table("companies")

        self.sector = self.db.table("sectors")

        self.results = []

        self.capital_records = []
        self.pnl = self.pnl.drop_duplicates(subset=["company_id", "year"], keep="first")

        self.bs = self.bs.drop_duplicates(subset=["company_id", "year"], keep="first")

        self.cf = self.cf.drop_duplicates(subset=["company_id", "year"], keep="first")
    ...
    def merged_dataframe(self):
        ...
        self.pnl["year"] = pd.to_numeric(self.pnl["year"], errors="coerce")
        ...
        company = self.company[
            ["id", "book_value", "roce_percentage", "roe_percentage"]
        ]

        sector = self.sector[["company_id", "broad_sector"]]
        ...
        duplicates = df.groupby(["company_id", "year"]).size().reset_index(name="count")
    ...
    def compute(self):
        ...
        for _, row in df.iterrows():
            ...
            check_opm_difference(opm, row.opm_percentage, company, year)

            EBIT = ebit(row.operating_profit, row.other_income)
            ...
            leverage_flag = high_leverage_flag(de, row.broad_sector)

            icr = interest_coverage_ratio(
                row.operating_profit, row.other_income, row.interest
            )

            icr_text = icr_label(row.interest, icr)

            icr_flag = icr_warning_flag(icr)

            debt = net_debt(row.borrowings, row.investments)

            turnover = asset_turnover(row.sales, row.total_assets)

            check_roce_difference(roce, row.roce_percentage, company, year)

            check_roe_difference(roe, row.roe_percentage, company, year)
            ...
            self.capital_records.append(
                {
                    "company_id": company,
                    "year": year,
                    "cfo_sign": cash_metrics["cfo_sign"],
                    "cfi_sign": cash_metrics["cfi_sign"],
                    "cff_sign": cash_metrics["cff_sign"],
                    "pattern_label": cash_metrics["pattern_label"],
                }
            )

            score = quality_score(roe, roce, npm, de)
            ...
            record = {
                ...
                "net_debt": debt,
                "free_cash_flow_cr": cash_metrics["free_cash_flow"],
                "fcf_conversion_pct": cash_metrics["fcf_conversion"],
                "capex_intensity_pct": cash_metrics["capex_intensity"],
                "capex_category": cash_metrics["capex_label"],
                "cfo_quality_score": cash_metrics["cfo_quality_score"],
                "cfo_quality_label": cash_metrics["cfo_quality_label"],
    ...
    def compute_cagr(self):
        ...
        for company in pnl["company_id"].unique():
            ...
            company_df = pnl[pnl["company_id"] == company].sort_values("year")

            revenue = company_df["sales"].tolist()
            pat = company_df["net_profit"].tolist()
            eps = company_df["eps"].tolist()

            result = compute_all_cagr(revenue, pat, eps)

            latest_year = company_df.iloc[-1]["year"]

            record = {"company_id": company, "year": latest_year}

            record.update(cagr_to_record(result))
    ...
    def run(self):
        ...
        final = ratios.merge(cagr, on=["company_id", "year"], how="left")
