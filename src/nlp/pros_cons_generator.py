...
for _, row in latest_ratios.iterrows():
    ...
    if (
        pd.notna(row["fcf_conversion_pct"])
        and row["fcf_conversion_pct"] > 80
    ):
    ...
    if (
        pd.notna(row["fcf_conversion_pct"])
        and row["fcf_conversion_pct"] < 50
    ):
        cons.append("Weak cash conversion")

    if (
        pd.notna(row["cfo_quality_score"])
        and row["cfo_quality_score"] < 40
    ):
        cons.append("Low earnings quality")

    if (
        pd.notna(row["capex_intensity_pct"])
        and row["capex_intensity_pct"] > 25
    ):
...
print(
    f"Companies processed: {len(pros_cons_df)}"
)

print(
    f"Output saved: output/pros_cons_generated.csv"
)
