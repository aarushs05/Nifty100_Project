...
def show():
    ...
    c4.metric(
        "Average FCF",
        f"{df['free_cash_flow_cr'].mean():,.0f} Cr"
    )
    ...
    with col1:
        ...
        st.write(
            f"ROE : {winner['return_on_equity_pct']:.2f}%"
        )

        st.write(
            f"ROCE : {winner['return_on_capital_employed_pct']:.2f}%"
        )

        st.write(
            f"Free Cash Flow : ₹ {winner['free_cash_flow_cr']:,.0f} Cr"
        )

        if "buyback_yield_pct" in df.columns:
            st.write(
                f"Buyback Yield : {winner['buyback_yield_pct']:.2f}%"
            )

        if "dividend_yield_pct" in df.columns:
            st.write(
                f"Dividend Yield : {winner['dividend_yield_pct']:.2f}%"
            )
    ...
    st.caption(
        "Nifty 100 Analytics Dashboard • Sprint 4 • Capital Allocation"
    )